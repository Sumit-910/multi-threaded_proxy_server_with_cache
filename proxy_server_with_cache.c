...
int handle_request(int client_socket, struct parsed_request *request, char *temp_req)
{
    ...
    {
        printf("Set header key doesn't work\n");
    }
    ...
    free(buf);
    add_cache_element(temp_buffer, strlen(temp_buffer), temp_req);
    printf("Done\n");
    free(temp_buffer);
...
cache_element *find(char *url)
{
    ...
    printf("Remove cache lock ulocked %d\n", temp_lock_val);
    return site;
}
